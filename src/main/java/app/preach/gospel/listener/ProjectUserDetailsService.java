...
public class ProjectUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(final String username) throws UsernameNotFoundException {
		...
		final var student = this.studentRepository.findActiveByUsernameOrEmail(username)
		...
		final var roleAuthByRoleId = role.authorities();
		...
		final List<Long> authIds = roleAuthByRoleId.stream().map(AuthorityRef::authId).toList();
		final List<Authority> authoritiesRecords = this.authorityRepository.findAllByIdIn(authIds);
