...
import app.preach.gospel.dao.AuthorityDao;
import app.preach.gospel.dao.RoleDao;
...
public class ProjectUserDetailsService implements UserDetailsService {
	...
	public ProjectUserDetailsService(final AuthorityDao authorityRepository, final RoleDao roleRepository,
			final StudentDao studentRepository) {
		this.authorityDao = authorityRepository;
	...
	public UserDetails loadUserByUsername(final String username) throws UsernameNotFoundException {
		...
		final var student = this.studentDao.selectActiveByUsernameOrEmail(username)
		...
		final var role = this.roleDao.selectByIdAndVisibleFlgTrue(student.roleId())
				.orElseThrow(() -> new AuthenticationCredentialsNotFoundException(
						ProjectConstants.MESSAGE_SPRINGSECURITY_LOGINERROR2));
		final List<RoleAuth> roleAuthByRoleId = this.roleDao.selectRoleAuthByRoleId(role.id());
		if (roleAuthByRoleId == null || roleAuthByRoleId.isEmpty()) {
		...
		final List<Long> authIds = roleAuthByRoleId.stream().map(RoleAuth::authId).toList();
		final List<Authority> authoritiesRecords = this.authorityDao.selectByIds(authIds);
