...
public interface HymnRepository extends ListCrudRepository<Hymn, Long> {
	...
	// 賛美歌を1件INSERT
	// ※ recordのネスト参照(:entity.nameJp)はSpring Data JDBCで未サポートのため
	// 各フィールドを個別@Paramで展開する
	@Modifying
	@Query("INSERT INTO HYMNS (ID, NAME_JP, NAME_KR, LINK, UPDATED_TIME, UPDATED_USER, LYRIC, VISIBLE_FLG, CLASSICAL) "
			+ "VALUES (:id, :nameJp, :nameKr, :link, :updatedTime, :updatedUser, :lyric, :visibleFlg, :classical)")
	void insertOne(@Param("id") Long id, @Param("nameJp") String nameJp, @Param("nameKr") String nameKr,
			@Param("link") String link, @Param("updatedTime") LocalDateTime updatedTime,
			@Param("updatedUser") Long updatedUser, @Param("lyric") String lyric,
			@Param("visibleFlg") String visibleFlg, @Param("classical") String classical);
}